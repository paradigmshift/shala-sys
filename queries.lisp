...
(defun get-pass-valid-on (year month student)
...
(defun pass-total-for (year month student)
  ...
  (let ((pass (get-pass-valid-on year month student)))
    (when pass
...
(defun students-with-pass-on (year month)
  "Returns a list of students with passes valid on given year and month"
  (remove-if-not #'(lambda (student)
                     (get-pass-valid-on year month student))
                 (students)))
...
(defun categorize-passes (year month student-list)
  ...
  (let ((morning '())
    ...
    (map 'list #'(lambda (student)
                   (let ((pass-type (get-type (get-pass-valid-on year month student))))
