...
;; Student functions
;;------------------------------------------------------------------------------
(defmacro defstudent-method-with-update (method-name (&rest args) &body body)
...
(defstudent-method-with-update new-drop-in (student drop-in)
  (push drop-in (drop-in student)))

(defstudent-method-with-update new-pass (student pass)
  (push pass (pass student))
  (setf (pass student)
        (sort-pass-dates (pass student))))

(defstudent-method-with-update remove-pass (student pass)
  (setf (pass student)
        (sort-pass-dates (remove (first (pass-list-dates->yy-mm-dd (list pass)))
                                 (pass-list-dates->yy-mm-dd (pass student)) :test #'equalp))))

(defstudent-method-with-update remove-drop-in (student drop-in)
...
(defstudent-method-with-update remove-last-pass (student)
...
(defstudent-method-with-update find-and-edit-pass (student pass pass-list new-pass)
...
(defstudent-method-with-update find-and-edit-drop-in (student pass pass-list new-pass)
  (multiple-value-bind (converted-pass converted-pass-list) (pass-and-pass-list-dates->yy-mm-dd pass pass-list)
    (when (member converted-pass converted-pass-list :test #'equalp)
      (progn
        (setf (elt pass-list (position converted-pass converted-pass-list :test #'equalp))
...
(defmethod remove-expense (expense)
  "Remove EXPENSE from the DB"
  (db.delete *expense-list* ($ ($ "comment" (comment expense))
                               ($ "date" (local->unix (date expense))))))
