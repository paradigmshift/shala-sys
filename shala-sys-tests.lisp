...
(defun populate-dummy-passes ()
  (progn 
    (new-pass (student-from-name "yukako kotani") (make-pass :type 'm :start-date (add-months -2 (time-now)))) ;; Has one pass that was bought 2 months ago (1 month type).
    (new-pass (student-from-name "kaori kawai") (make-pass :type 'w :start-date (add-days -6 (time-now)))) ;; Has one pass that was bought 6 days ago (1 week type).
    (new-pass (student-from-name "yoko kamioka") (make-pass :type 'm :start-date (time-now))) ;; Has one pass that was bought now (1 month type)
